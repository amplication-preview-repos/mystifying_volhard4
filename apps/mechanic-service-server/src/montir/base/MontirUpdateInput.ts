/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CustomerOrderUpdateManyWithoutMontirsInput } from "./CustomerOrderUpdateManyWithoutMontirsInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class MontirUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CustomerOrderUpdateManyWithoutMontirsInput,
  })
  @ValidateNested()
  @Type(() => CustomerOrderUpdateManyWithoutMontirsInput)
  @IsOptional()
  @Field(() => CustomerOrderUpdateManyWithoutMontirsInput, {
    nullable: true,
  })
  customerOrders?: CustomerOrderUpdateManyWithoutMontirsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  montirName?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  montirPassword?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  montirPhone?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  section?: string | null;
}

export { MontirUpdateInput as MontirUpdateInput };
